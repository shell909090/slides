...
PS: 根据我最新的代码研读结果，并不是加锁来解决这个问题的。这个问题是否真正的在内核被解决了，我目前还存疑。等我搞明白了再补充。
...
实际上，这也是不成立的。线程在什么时候发生陷入切换？正常情况下，应当是IO阻塞的时候。同样的IO量，难道其他模型就不需要陷入了么？只是非阻塞模型有很大可能直接返回，并不发生上下文切换而已。从原理上说，由于非阻塞模式调用可能失败，因此内核陷入开销的影响反而更大。
